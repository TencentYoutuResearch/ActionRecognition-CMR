...
from utils.log_helper import init_log, get_log_format
from utils.optimizer_helper import build_optimizer
from utils.metrics import Top1Metric
from utils.utils import format_cfg, accuracy, AverageMeter, load_checkpoint
from utils.dist_helper import get_rank, get_world_size, all_reduce
from utils.model_helper import load_state_dict
from utils.dataset_helper import get_val_crop_transform, get_dataset, shuffle_dataset
from core.models_entry import get_model, get_augmentation
...
class TemporalHelper(object):
    ...
    def adjust_learning_rate(self, optimizer, epoch, lr_type, lr_steps):
        ...
        if lr_type == 'MultiStepLR':
            decay = 0.1 ** (sum(epoch >= np.array(lr_steps)))
            lr = self.config.trainer.optimizer.kwargs['lr'] * decay
            decay = self.config.trainer.optimizer.kwargs['weight_decay']
        elif lr_type == 'CosineAnnealingLR':
            import math
            lr = 0.5 * self.config.trainer.optimizer.kwargs['lr'] * (
                    1 + math.cos(math.pi * epoch / self.config.trainer.epochs))
            decay = self.config.trainer.optimizer.kwargs['weight_decay']
        else:
    ...
    def _resume(self, ckpt=None):
        ...
        # load model weights
        load_state_dict(self.model, ckpt['model'], strict=True)
        if not self.inference_only:
            if 'optimizer' in ckpt:
                ...
                # self.lr_scheduler.last_epoch = self.start_iter
                self.logger.info(f'resume from epoch:{self.start_epoch}')
            if 'states' in ckpt:
                print("load random states")
                states = ckpt['states']
    ...
    def _build_dataloader(self, data_type):
        ...
        if data_type == 'train':
            ...
            transform = torchvision.transforms.Compose([
                ...
                normalize,
                ConvertDataFormat(self.config.net.model_type, channel),
            ])
            ...
            crop_aug = torchvision.transforms.Compose([GroupScale(int(dargs.scale_size)),
                                                       get_val_crop_transform(self.config.dataset, spatial_crops)])
            transform = torchvision.transforms.Compose([
            ...
            # torch.distributed.init_process_group('nccl', init_method='env://',world_size=self.world_size,rank=0)
            sampler = DistributedSampler(dataset) if self.config.gpus > 1 else None
    ...
    def _setup_env(self):
        ...
        random.seed(self.config.get('seed', 2020))
        torch.manual_seed(self.config.get('seed', 2020))
        torch.cuda.manual_seed(self.config.get('seed', 2020))
        torch.cuda.manual_seed_all(self.config.get('seed', 2020))
        ...
        if self.rank == 0 and not self.inference_only:
            self.tb_logger = SummaryWriter(os.path.join(self.work_dir, 'tensorboard'))
    ...
    def update(self):
        ...
        self.adjust_learning_rate(self.optimizer, self.cur_epoch, self.config.trainer.lr_scheduler.type,
                                  self.config.trainer.lr_scheduler.kwargs.get('milestones', False))
    ...
    def train(self):
        ...
        for iter_idx in range(self.start_iter, self.max_iter):
            ...
            inputs, offset = self.get_batch('train')
            ...
            if iter_idx % self.config.trainer.print_freq == 0 and self.rank == 0:
                ...
                self.tb_logger.add_scalar('lr', self.optimizer.param_groups[-1]['lr'], iter_idx)
                ...
                if self.multi_class:
                    ...
                    self.logger.info(log_formatter.format(
                        ...
                        batch_time=self.metrics['batch_time'], data_time=self.metrics['data_time'],
                        loss=self.metrics['losses'],
                        mAP=self.metrics['mAP'], lr=self.optimizer.param_groups[-1]['lr']))
                else:
                    ...
                    # print(inputs[0].mean(), inputs[0].std())
                    self.logger.info(log_formatter.format(
                        iter_idx, self.max_iter, self.cur_epoch + 1, self.config.trainer.epochs,
                        batch_time=self.metrics['batch_time'], data_time=self.metrics['data_time'],
                        loss=self.metrics['losses'],
                        top1=self.metrics['top1'], top5=self.metrics['top5'], lr=self.optimizer.param_groups[-1]['lr']))
    ...
    def evaluate(self):
        ...
        for iter_idx in range(test_len):
            inputs, offset = self.get_batch('val')
